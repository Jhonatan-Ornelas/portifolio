...
function priceToNumber(priceString) {
    const withoutThousandSeparator = priceString.replace(/\./g, '');
...
for (let i of products.data) {
    ...
    // price atribute
    card.setAttribute('data-price', priceToNumber(i.priceP));
    //img div
...
//ordena os produtos
let field = document.querySelector('#produtos-cards')
let div = Array.from(field.children);
let select = document.getElementById('select')
ar = Array.from(field.children);
...
function sortingValue(value) {
    ...
    if (value === 'LowToHigh') {
        sortElem(field, div, true, 'price');
    }
    if (value === 'HighToLow') {
        sortElem(field, div, false, 'price');
    }
    if (value === 'AToZ') {
        sortElem(field, div, true, 'name');
    }
    if (value === 'ZToA') {
        sortElem(field, div, false, 'name');
    }
}

function sortElem(field, div, asc, sortBy) {
    let dm, sortDiv;
    dm = asc ? 1 : -1;
    sortDiv = div.sort((a, b) => {
        let ax, bx;
        if (sortBy === 'price') {
            ax = priceToNumber(a.getAttribute('data-price'));
            bx = priceToNumber(b.getAttribute('data-price'));
        } else if (sortBy === 'name') {
            ax = a.querySelector('.item-decricao').textContent;
            bx = b.querySelector('.item-decricao').textContent;
        }
...
document.addEventListener('click', function (e) {
  ...
  document.querySelectorAll('.custom-select-options li').forEach(function (li) {
    li.addEventListener('click', function () {
        ...
        customSelect.querySelector('.custom-select-selected').textContent = text;
        customSelect.setAttribute('data-value', value);
        closeCustomSelect();

        // Chame sua função de ordenação aqui
        sortingValue(value);
    });
});
